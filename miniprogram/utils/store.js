/**
 * 拾途 Paper Trip · 数据层（微信小程序版）
 * 与网页版共用同一份数据结构与存储键名（xingji.trip.v1），导入/导出的 JSON 双向互通。
 *
 * 数据结构：
 *   state = {
 ...
 *     days:   [{ id, name, date, items: [{ id, placeId, time, stay, cost, note, disabled, leg: { mode, note } }] }],
 *     settings: { mapStyle, viewMode, amapKey, satellite, view3D },
 *     view: { lng, lat, zoom }
 *   }
 */
(function () {
    ...
    /** 旧数据迁移：公交/地铁合并为「公共交通」transit；已移除的「动车」并入「其他」 */
    ...
    function createEmptyState() {
        return {
            ...
            settings: { mapStyle: 'dark', viewMode: '2D', amapKey: '', satellite: false, view3D: false },
    ...
    function normalizeState(raw) {
        ...
        if (raw.settings) {
            ...
            state.settings.amapKey = String(raw.settings.amapKey || '').trim();
            state.settings.satellite = raw.settings.satellite === true;
            state.settings.view3D = raw.settings.view3D === true;
        }
    ...
    function readRaw() {
        var raw = null;
        try { raw = wx.getStorageSync(STORAGE_KEY); } catch (e) { raw = null; }
        if (!raw) return null;
    ...
    function writeRaw(text) {
        try { wx.setStorageSync(STORAGE_KEY, text); } catch (e) { /* 存储异常忽略 */ }
    }
    ...
    var Store = {
        ...
        persist: function () {
            writeRaw(JSON.stringify(this.state));
        },

        saveSoon: function () {
            var self = this;
            if (saveTimer) clearTimeout(saveTimer);
            saveTimer = setTimeout(function () { self.persist(); }, 250);
        },
        ...
        addPlace: function (data) {
            var place = {
                ...
                lng: num(data.lng, 0),
                lat: num(data.lat, 0),
                categoryId: normalizeCategoryId(data.categoryId || 'activity'),
        ...
        updatePlace: function (id, patch) {
            ...
            if (patch.categoryId !== undefined) place.categoryId = normalizeCategoryId(patch.categoryId);
